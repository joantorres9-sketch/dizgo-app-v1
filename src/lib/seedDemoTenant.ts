...
// Sembrador de datos demo por país — extraído de ejecutarSeed() en
// src/app/dashboard/admin/page.tsx para poder reutilizarlo tanto desde el botón manual de
// Superadmin (cliente browser, RLS del propio tenant) como desde la creación server-side de
// usuarios de cortesía (cliente admin/service-role, tenant recién creado). La lógica de negocio
// es idéntica a la original, solo se parametrizó por cliente/tenant/país y se reemplazó el
// setState de la página por un callback onProgress opcional.

export const CONFIG_PAIS: Record<string, {
...
export type OnProgress = (pasoKey: string, estado: EstadoPaso, mensaje?: string) => void
...
async function insertarPaso(
  ...
  if (error) {
    onProgress?.(pasoKey, 'error', `❌ ${tabla}: ${error.message}`)
    return false
  }
  onProgress?.(pasoKey, 'ok', mensajeExito)
  return true
}

export async function sembrarTenantDemo(
  supabase: SupabaseClient,
  tenantId: string,
  paisCodigo: string,
  onProgress?: OnProgress
  ...
  // ── PRODUCTOS ──────────────────────────────────────────────
  onProgress?.('productos', 'cargando', `🛍️ Cargando ${cfg.productos.length} productos para ${cfg.nombre}...`)
  ...
  if (prodsError) {
    onProgress?.('productos', 'error', `❌ Error insertando productos: ${prodsError.message}`)
    return
  }

  const prodIds = (prodsInserted || []).map((p: { id: string; nombre: string; pvp_final: number; costo_proveedor: number }) => ({
    id: p.id, nombre: p.nombre, pvp: p.pvp_final, costo: p.costo_proveedor,
  }))

  if (prodIds.length === 0) {
    onProgress?.('productos', 'error', '❌ No se crearon productos. Verifica que no existan duplicados o que RLS lo permita.')
    return
  }
  onProgress?.('productos', 'ok', `✅ ${prodIds.length} productos creados`)

  // ── COSTOS FIJOS ──────────────────────────────────────────
  onProgress?.('costos', 'cargando', '📊 Cargando costos fijos 6 meses...')
  const cfRows = []
  for (let i = 5; i >= 0; i--) {
  ...
  if (cfErr) onProgress?.('costos', 'error', `❌ Costos fijos: ${cfErr.message}`)
  else onProgress?.('costos', 'ok', `✅ ${cfRows.length} registros de costos fijos`)

  // ── PEDIDOS ───────────────────────────────────────────────
  onProgress?.('pedidos', 'cargando', '📦 Generando pedidos 6 meses...')
  let totalPedidos = 0
  ...
  for (let mes = 5; mes >= 0; mes--) {
    ...
    if (pedErr) { onProgress?.('pedidos', 'error', `❌ Pedidos: ${pedErr.message}`); huboErrorPedidos = true; break }
    totalPedidos += pedidosBatch.length
  }
  if (!huboErrorPedidos) onProgress?.('pedidos', 'ok', `✅ ${totalPedidos} pedidos generados`)

  // ── PAUTA ─────────────────────────────────────────────────
  onProgress?.('pauta', 'cargando', '📡 Cargando campañas Meta Ads...')
  const pautaRows = []
  for (let mes = 5; mes >= 0; mes--) {
  ...
  if (pautaErr) onProgress?.('pauta', 'error', `❌ Pauta: ${pautaErr.message}`)
  else onProgress?.('pauta', 'ok', `✅ ${pautaRows.length} campañas cargadas`)

  // ── WALLET ────────────────────────────────────────────────
  onProgress?.('wallet', 'cargando', '💳 Cargando movimientos de wallet...')
  const walletRows = []
  for (let mes = 5; mes >= 0; mes--) {
    const fecha = new Date(hoy.getFullYear(), hoy.getMonth() - mes, 1)
  ...
  await insertarPaso(supabase, 'wallet_transacciones', walletRows, 'wallet', `✅ ${walletRows.length} movimientos de wallet`, onProgress)

  // ── LIBRO DE CAJA ─────────────────────────────────────────
  onProgress?.('libro_caja', 'cargando', '📒 Cargando libro de caja...')
  const cajaRows = []
  for (let mes = 5; mes >= 0; mes--) {
    const fecha = new Date(hoy.getFullYear(), hoy.getMonth() - mes, 1)
  ...
  await insertarPaso(supabase, 'libro_caja', cajaRows, 'libro_caja', `✅ ${cajaRows.length} movimientos en libro de caja`, onProgress)

  // ── METAS ─────────────────────────────────────────────────
  onProgress?.('metas', 'cargando', '🎯 Cargando metas 6 meses...')
  const metasRows = []
  for (let mes = 5; mes >= 0; mes--) {
    const fecha = new Date(hoy.getFullYear(), hoy.getMonth() - mes, 1)
    const periodo = `${fecha.getFullYear()}-${String(fecha.getMonth() + 1).padStart(2, '0')}-01`
  ...
  await insertarPaso(supabase, 'metas', metasRows, 'metas', `✅ ${metasRows.length} metas creadas`, onProgress)

  // ── BODEGAS ───────────────────────────────────────────────
  onProgress?.('bodegas', 'cargando', '🏭 Creando bodegas e inventario...')
  ...
  if (bodErr) {
    onProgress?.('bodegas', 'error', `❌ Bodegas: ${bodErr.message}`)
  } else {
    if (bodegasData && prodIds.length > 0) {
      ...
      if (invErr) onProgress?.('bodegas', 'error', `❌ Inventario: ${invErr.message}`)
    }
    onProgress?.('bodegas', 'ok', '✅ 2 bodegas + inventario inicial')
  }
  ...
  onProgress?.('pqrsf', 'cargando', '📬 Generando casos PQRSF...')
  ...
  if (pqrsfErr) onProgress?.('pqrsf', 'error', `❌ PQRSF: ${pqrsfErr.message}`)
  else onProgress?.('pqrsf', 'ok', '✅ 15 casos PQRSF generados')

  // ── ALERTAS ───────────────────────────────────────────────
  onProgress?.('alertas', 'cargando', '🚨 Generando alertas de demostración...')
  await insertarPaso(supabase, 'alertas', [
  ...
  ], 'alertas', '✅ 8 alertas de demostración creadas', onProgress)

  // ── NÓMINA — colaboradores + tasas por país ───────────────
  onProgress?.('nomina', 'cargando', '👥 Cargando colaboradores y tasas de nómina...')
  ...
  if (colabErr || tasasErr) onProgress?.('nomina', 'error', `❌ Nómina: ${colabErr?.message || tasasErr?.message}`)
  else onProgress?.('nomina', 'ok', '✅ 2 colaboradores + tasas de nómina cargadas')

  // ── INVERSIÓN — activos fijos + crédito ───────────────────
  onProgress?.('inversion', 'cargando', '💰 Cargando activos fijos e inversiones...')
  ...
  if (activosErr || creditoErr) onProgress?.('inversion', 'error', `❌ Inversión: ${activosErr?.message || creditoErr?.message}`)
  else onProgress?.('inversion', 'ok', '✅ 3 activos fijos + 1 crédito de capital de trabajo')

  // ── CUENTAS POR PAGAR ────────────────────────────────────
  onProgress?.('cxp', 'cargando', '📋 Cargando cuentas por pagar...')
  ...
  await insertarPaso(supabase, 'cuentas_por_pagar', [
  ...
  ], 'cxp', '✅ 5 cuentas por pagar (1 vencida para demo)', onProgress)

  // ── METAS SEGUIMIENTO DIARIO — 30 días ───────────────────
  onProgress?.('metas_diarias', 'cargando', '📅 Cargando seguimiento diario 30 días...')
  const metasDiarias = []
  ...
  await insertarPaso(supabase, 'metas_seguimiento_diario', metasDiarias, 'metas_diarias', '✅ 30 días de seguimiento diario cargados', onProgress)

  // ── NÓMINA — procesos organizacionales ───────────────────
  onProgress?.('nomina_procesos', 'cargando', '🏢 Cargando procesos organizacionales...')
  await insertarPaso(supabase, 'nomina_procesos', [
  ...
  ], 'nomina_procesos', '✅ 3 procesos organizacionales creados', onProgress)

  // ── INVERSIÓN — capital propio + socios ──────────────────
  onProgress?.('inversion_capital', 'cargando', '💼 Cargando capital propio e inversión de socios...')
  ...
  if (capitalErr || sociosErr) onProgress?.('inversion_capital', 'error', `❌ Capital/socios: ${capitalErr?.message || sociosErr?.message}`)
  else onProgress?.('inversion_capital', 'ok', '✅ Capital propio + socio fundador registrados')

  // ── PUNTO DE EQUILIBRIO — configuración ──────────────────
  onProgress?.('equilibrio', 'cargando', '⚖️ Configurando punto de equilibrio...')
  ...
  onProgress?.('equilibrio', 'ok', `✅ PE configurado: mínimo ${peMinimo} pedidos/mes para ${cfg.nombre}`)

  // ── WHATSAPP / CENTRO CONTACTO ────────────────────────────
  onProgress?.('whatsapp', 'cargando', '💬 Configurando Centro de Contacto WhatsApp...')
  ...
  await insertarPaso(supabase, 'whatsapp_templates_config', [
    {
  ...
  ], 'whatsapp', '✅ 5 templates WhatsApp + contexto de tienda configurados', onProgress)

  // ── LOGÍSTICA — transportadoras + novedades IA ────────────
  onProgress?.('logistica', 'cargando', '🚚 Cargando transportadoras y scripts de novedades...')
  ...
  if (!transExist || transExist === 0) {
    ...
    if (transErr) onProgress?.('logistica', 'error', `❌ Transportadoras: ${transErr.message}`)
  }
  ...
  if (!novExist || novExist === 0) {
    ...
    if (novErr) onProgress?.('logistica', 'error', `❌ Novedades IA: ${novErr.message}`)
  }

  onProgress?.('logistica', 'ok', `✅ Sembrado completo para ${cfg.nombre}`)
}
