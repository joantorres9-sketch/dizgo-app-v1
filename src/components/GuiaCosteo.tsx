...
export function GuiaCosteo({ tenantId }: { tenantId: string }) {
  ...
  if (setupCompleto) {
    ...
    return (
      <div className="no-print" style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.green}`, borderRadius: '12px', padding: '14px 18px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: T.text }}>🎉 Ya tienes lo básico de tu costeo listo</div>
          <div style={{ fontSize: '11.5px', color: T.muted, marginTop: '2px' }}>Ahora toca verificar: simula tu precio y revisa tu punto de equilibrio con datos reales.</div>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          <Link href="/dashboard/precio" style={{ textDecoration: 'none', fontSize: '11.5px', fontWeight: 600, color: T.accent, background: `${T.accent}15`, border: `1px solid ${T.accent}30`, borderRadius: '8px', padding: '7px 12px' }}>💡 Simular precio</Link>
          <Link href="/dashboard/equilibrio" style={{ textDecoration: 'none', fontSize: '11.5px', fontWeight: 600, color: T.accent, background: `${T.accent}15`, border: `1px solid ${T.accent}30`, borderRadius: '8px', padding: '7px 12px' }}>⚖️ Punto de equilibrio</Link>
          <button onClick={ocultarAvanzada} style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer', fontSize: '13px' }}>✕</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="no-print" style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.accent}`, borderRadius: '12px', padding: '16px 18px', marginBottom: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px', gap: '10px' }}>
        <div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: T.text }}>🧭 Guía de costeo -- {completados}/{pasos.length} pasos</div>
          <div style={{ fontSize: '11.5px', color: T.muted, marginTop: '2px' }}>Vender es la parte fácil. Aquí te guiamos, paso a paso, a saber cuánto ganas de verdad en cada venta.</div>
        </div>
        <button onClick={ocultarPorAhora} style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer', fontSize: '13px', flexShrink: 0, whiteSpace: 'nowrap' }}>✕ Ocultar por ahora</button>
      </div>
      <div style={{ display: 'grid', gap: '6px' }}>
        {pasos.map(p => (
          <Link key={p.key} href={p.href} style={{ textDecoration: 'none', display: 'flex', gap: '10px', alignItems: 'flex-start', padding: '9px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px' }}>
            ...
            <div>
              <div style={{ fontSize: '11.5px', fontWeight: 600, color: p.hecho ? T.green : T.text }}>{p.label}</div>
              <div style={{ fontSize: '10.5px', color: T.muted, lineHeight: 1.4 }}>{p.detalle}</div>
            </div>
          </Link>
        ))}
      </div>
