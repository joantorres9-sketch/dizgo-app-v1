    ...
    try {
      const { data: prodsInserted, error: prodsError } = await supabase.from('productos').insert(
        cfg.productos.map(p => ({
          ...
          costo_proveedor: p.costo, pvp_final: p.pvp,
          costo_flete: Math.round(p.costo * 0.25),
          costo_flete_dev: Math.round(p.costo * 0.3),
          costo_fulfillment: 0, costo_full_dev: 0,
          cf_pedido: Math.round(p.pvp * 0.03),
          pct_publicidad: 20, pct_pub_dev: 7, pct_pub_cancel: 4,
          pct_desc_popup: 2, pct_com_plataforma: 0, pct_pasarela: 3.49,
          pct_com_pasarela: 0, pct_com_ventas: 5, pct_com_admin: 2,
