...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!cargando && perfil && !enHorario) {
    return (
      <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', background: T.bg, color: T.text, fontFamily: 'system-ui,sans-serif', padding: '20px' }}>
        <div style={{ textAlign: 'center', maxWidth: '380px' }}>
          <div style={{ fontSize: '17px', fontWeight: 700, marginBottom: '10px' }}>⏰ Fuera de horario</div>
          <div style={{ fontSize: '13px', color: T.muted, lineHeight: 1.6, marginBottom: '18px' }}>
          ...
          <button onClick={cerrarSesion} style={{ padding: '9px 18px', background: 'rgba(240,92,92,0.1)', border: '1px solid rgba(240,92,92,0.3)', borderRadius: '8px', color: T.red, fontWeight: '600', fontSize: '12px', cursor: 'pointer' }}>
            🚪 Cerrar sesión
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:T.bg, color:T.text, fontFamily:'system-ui,sans-serif' }}>
      ...
      <aside className={`dz-sidebar${menuOpen?' open':''}`} style={{ width:'220px', background:T.card2, borderRight:`1px solid ${T.border}`, display:'flex', flexDirection:'column', position:'fixed', top:0, left:0, height:'100vh', overflowY:'auto', flexShrink:0 }}>
        <div style={{ padding:'16px', borderBottom:`1px solid ${T.border}` }}>
          <Link href="/dashboard" style={{ textDecoration:'none', display:'flex', alignItems:'center', gap:'10px' }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/brand/dizgo-icon.png" alt="DIZGO" width={36} height={36} style={{ borderRadius:'8px', flexShrink:0 }} />
            <div>
              <div style={{ fontWeight:'800', fontSize:'16px', color:T.text }}>d<span style={{ color:T.accent }}>i</span>zgo</div>
              <div style={{ fontSize:'9px', color:T.muted }}>Hallazgo de dinero</div>
            </div>
          </Link>
        </div>
        <nav style={{ flex:1, padding:'8px 6px' }}>
          <div style={{ marginBottom:'14px' }}>
            {(() => {
              ...
              return (
                <Link key={item.href} href={item.href} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'7px 10px', borderRadius:'8px', marginBottom:'2px', textDecoration:'none', background: active ? `${T.blue}15` : 'transparent', borderLeft: active ? `2px solid ${T.blue}` : '2px solid transparent', color: active ? T.blue : T.muted, fontSize:'13px', fontWeight: active ? '600' : '400' }}>
          ...
          {gruposConItems.map(group => (
            <div key={group.key} style={{ marginBottom:'14px' }}>
              ...
              {group.items.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'7px 10px', borderRadius:'8px', marginBottom:'2px', textDecoration:'none', background: active ? `${group.color}15` : 'transparent', borderLeft: active ? `2px solid ${group.color}` : '2px solid transparent', color: active ? group.color : T.muted, fontSize:'13px', fontWeight: active ? '600' : '400' }}>
                    <span style={{ fontSize:'15px', width:'18px', textAlign:'center', flexShrink:0 }}>{item.icon}</span>
                    <span>{item.label}</span>
                  </Link>
                )
          ...
          {itemsSuperadmin.length > 0 && (
            <div style={{ marginBottom:'14px' }}>
              <div style={{ padding:'4px 10px 6px', fontSize:'9px', fontWeight:'800', letterSpacing:'1.5px', color:T.purple }}>ACTUAR</div>
              {itemsSuperadmin.map(item => {
                const active = pathname === item.href || pathname.startsWith(item.href)
                return (
                  <Link key={item.href} href={item.href} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'7px 10px', borderRadius:'8px', marginBottom:'2px', textDecoration:'none', background: active ? `${T.purple}15` : 'transparent', borderLeft: active ? `2px solid ${T.purple}` : '2px solid transparent', color: active ? T.purple : T.muted, fontSize:'13px', fontWeight: active ? '600' : '400' }}>
                    <span style={{ fontSize:'15px', width:'18px', textAlign:'center', flexShrink:0 }}>{item.icon}</span>
                    <span>{item.label}</span>
                  </Link>
                )
        ...
        <div style={{ padding:'12px', borderTop:`1px solid ${T.border}`, display:'flex', flexDirection:'column', gap:'8px' }}>
          <SelectorTema />
          <div style={{ fontSize:'11px', color:T.muted, textAlign:'center' }}>DIZGO v2.0 · COP</div>
          <button onClick={cerrarSesion} disabled={saliendo} style={{ width:'100%', display:'block', textAlign:'center', padding:'8px', background:'rgba(240,92,92,0.1)', border:'1px solid rgba(240,92,92,0.3)', borderRadius:'8px', color:T.red, fontWeight:'600', fontSize:'12px', cursor: saliendo ? 'wait' : 'pointer' }}>
      ...
      <div className="dz-main" style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        <div
          style={{ padding:'10px 14px', borderBottom:`1px solid ${T.border}`, alignItems:'center', gap:'10px', position:'sticky', top:0, background:T.bg, zIndex:30 }}
          className="dz-hamburger"
        >
          <button onClick={()=>setMenuOpen(o=>!o)} aria-label="Abrir menú"
            style={{ background:T.card2, border:'none', borderRadius:'8px', width:'36px', height:'36px', color:T.text, fontSize:'18px', cursor:'pointer', flexShrink:0 }}>
            ☰
          </button>
          <div style={{ fontWeight:'800', fontSize:'15px', color:T.text, flex:1 }}>d<span style={{ color:T.accent }}>i</span>zgo</div>
          <SelectorTema compacto />
        </div>
        <main style={{ flex:1, padding:'28px', minWidth:0, color:T.text }}>{children}</main>
      </div>
    </div>
  )
}
